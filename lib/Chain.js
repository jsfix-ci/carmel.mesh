...
class Chain {
    constructor(session) {
        ...
        this._provider = (0, src_1.anonChain)({ url: this.url || DEFAULT_URL });
    }
    get session() {
    ...
    async _fetchIdentity(username) {
        if (!username)
            return;
        const result = await (0, src_1.getId)(this.provider, username);
        return result;
    }
    async _fetchRelays() {
    ...
    get op() {
        ...
        return {
            system: (call, data, key = 'main') => (0, src_1.system)(this.provider, call, data, key)
        };
    }
}
exports.Chain = Chain;
