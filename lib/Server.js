...
const src_1 = require("@carmel/eos/src");
...
class Server {
    ...
    async connectToEOS() {
        if (this.session.config.eos) {
            this._chain = Object.assign({ account: this.session.config.eos.keys.main.id }, eos.chain({
        ...
        this._chain = Object.assign({}, eos.anonChain({ url: src_1.DEFAULT_URL }));
    }
    get _() {
        return {
            getId: (username) => eos.getId(this.chain, username),
            system: async (action, data, key = 'main') => eos.system(this.chain, action, data, key)
        };
    }
    async start(ipfs) {
